import type { AnalysisResult, TranscriptEvent } from "@vibeinsights/shared";
...
export function analyzeTranscript(events: TranscriptEvent[]): AnalysisResult {
  ...
  if (metrics.retries > 2) {
    recommendations.push("Consider adding error handling or validation before tool calls");
  }

  if (metrics.contextOverflows > 0) {
    recommendations.push("Use Grep or Read with limits to avoid large file reads");
  }

  if (toolFailureRate > 0.3) {
    recommendations.push("Review tool usage patterns and check for common error causes");
  }
...
function detectContextOverflows(events: TranscriptEvent[]): number {
  return events.filter(
    (e): e is Extract<TranscriptEvent, { type: "tool_result" }> =>
      e.type === "tool_result" &&
      e.error !== undefined &&
      (e.error.includes("context") || e.error.includes("token limit") || e.error.includes("too large")),
  ).length;
}
