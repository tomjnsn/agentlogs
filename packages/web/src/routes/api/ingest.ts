  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const existingTranscript = await (async () => {
          if (repoId) {
        ...
        // All validations passed - now start uploads to storage
        // For private transcripts (no repo), use "private/<userId>" as prefix
        const storageKeyPrefix = repoId ? `${repoId}/${transcriptId}` : `private/${userId}/${transcriptId}`;
        ...
        // Upload unified transcript to storage
        const unifiedJson = JSON.stringify(unifiedTranscript);
        const storageUploadsPromise = (async () => {
          await storage.put(`${storageKeyPrefix}.json`, unifiedJson, {
            httpMetadata: { contentType: "application/json" },
          });
          logger.debug("Uploaded unified transcript to storage", {
            key: storageKeyPrefix,
            unifiedSize: unifiedJson.length,
          });
        })();

        // Upload validated blobs to storage in parallel
        const blobUploadsPromise =
          validatedBlobs.length > 0
            ? Promise.all(
                validatedBlobs.map(async ({ sha256: blobSha256, data, mediaType }) => {
                  const storageKey = `blobs/${blobSha256}`;
                  const existing = await storage.head(storageKey);
                  if (!existing) {
                    await storage.put(storageKey, data, { httpMetadata: { contentType: mediaType } });
                    logger.debug("Uploaded blob to storage", { key: storageKey, size: data.byteLength });
                  }
                }),
              )
            : Promise.resolve();

        // Wait for summary and storage uploads to complete
        const [summary] = await Promise.all([summaryPromise, storageUploadsPromise, blobUploadsPromise]);
