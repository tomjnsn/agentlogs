...
Cline supports [file-based hooks](https://docs.cline.bot/features/hooks/hook-reference) — executable scripts that run at key points in a task's lifecycle. The install command creates two hooks:
...
Each hook receives JSON from Cline via stdin containing the `taskId`, then calls `agentlogs cline upload` in the background to convert and upload the transcript from `~/.cline/data/tasks/`.
